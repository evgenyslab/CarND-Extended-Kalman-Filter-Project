...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  // Get change in time: expressed in seconds
  float dt = (measurement_pack.timestamp_ - previous_timestamp_) / 1000000.0;
  // Save previous time stamp:
	previous_timestamp_ = measurement_pack.timestamp_;

  // Time variables for covariance matrix:
  float dt_2 = dt * dt;
  float dt_3 = dt_2 * dt;
  float dt_4 = dt_3 * dt;
  ...
  if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
    ...
    // set ekf measurement covariance matrix to radar:
    ekf_.R_ = R_radar_;
    // Radar updates
    ...
    // set laser H:
    ekf_.H_ = H_laser_;
    // set ekf measurement covariance matrix to laser:
    ekf_.R_ = R_laser_;
    // Laser updates
